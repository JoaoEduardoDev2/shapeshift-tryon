...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!store_id || !access_token) {
      return new Response(
        JSON.stringify({ error: "store_id and access_token are required" }),
    ...
    await supabase.from("store_settings").upsert(
      { user_id: userId, platform: "nuvemshop", platform_url: `https://www.nuvemshop.com.br/loja/${store_id}`, store_name: `nuvemshop-${store_id}` },
      { onConflict: "user_id" }
    );
    ...
    console.error("nuvemshop-import error:", err);
    return new Response(
